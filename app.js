...
function save(key, val) {
  try {
    ...
    console.warn('⚠️ localStorage plein — nettoyage en cours…');
    cleanOldMedia();
    try {
      localStorage.setItem(key, JSON.stringify(val));
    } catch(e2) {
      // Dernier recours : vider les anciens listings (garder 10 derniers)
      if (key === DB_KEY) {
...
function cleanOldMedia() {
  // Libérer de l'espace : garder seulement la 1ère image par listing
  ...
  console.warn('🧹 Nettoyage auto médias effectué');
}
...
function initApp() {
  // Initialiser EmailJS dès le démarrage
...
function sendAdminEmail(u) {
  // Initialiser EmailJS si pas encore fait
  if (typeof emailjs !== 'undefined') {
    try { emailjs.init(EMAILJS_PUBLIC_KEY); } catch(e) {}
    ...
    console.log('📧 [Admin] Nouvel inscrit → immobiliersn9@gmail.com', u);
  }
}
...
/* Media upload — PERSISTÉ EN BASE64
   Correction 1 : limites 500MB vidéo / 20MB image
   Correction 4 : réindexation après suppression */
function handleMediaUpload(event) {
  ...
  files.forEach(file=>{
    ...
    if (file.size > maxSize) {
      showToast('Fichier trop grand (max '+maxLabel+') : '+file.name, 'error');
      return;
    }
    ...
    reader.onload = e => {
      ...
      renderMediaThumbs(preview);   // Réindexer tous les boutons
    };
...
/* Correction 4 : Réindexation complète après chaque ajout/suppression */
function renderMediaThumbs(preview) {
  ...
  uploadedMedia.forEach((m, idx) => {
    ...
    if (m.type === 'video') {
      thumb.innerHTML = '<video src="'+m.dataUrl+'" muted preload="metadata" style="width:100%;height:100%;object-fit:cover"></video>'
      ...
      thumb.innerHTML = '<img src="'+m.dataUrl+'" style="width:100%;height:100%;object-fit:cover"/>'
        +'<button class="remove-media" onclick="removeMedia('+idx+')"><i class="fas fa-times"></i></button>';
    }
...
/* Correction 4 : removeMedia re-render complet pour bonne réindexation */
...
function submitPublication() {
  ...
  // Correction 6 : vérifier qu'il y a au moins un média OU du texte
  ...
  if (!hasMedia && !hasText) {
    showToast('Ajoutez au moins une photo, une vidéo ou du texte','error');
    return;
  }
  ...
  setTimeout(()=>{
    ...
    const newListing = {
      ...
      location, zone:location.toLowerCase(), description:desc,
    ...
    let saved = false;
    try { saveListings(); saved=true; } catch(e) {
      hideLoading();
      showToast('Stockage plein. Réduisez la taille des médias.','error');
      allListings.shift(); // Annuler l'ajout
      return;
    }

    if (saved) {
      // Correction 7 : vider uploadedMedia après publication réussie
      uploadedMedia = [];
      renderMediaThumbs();
      hideLoading();
...
async function callDeepSeek(prompt) {
  // Correction 8 : gestion propre des erreurs API
  try {
    const res = await fetch(DEEPSEEK_URL, {
      method: 'POST',
    ...
    if (!res.ok) {
      ...
      throw new Error('DeepSeek HTTP '+res.status+': '+errText.substring(0,100));
    }
    const data = await res.json();
    return data.choices?.[0]?.message?.content || '{}';
  } catch(e) {
    console.warn('DeepSeek search fallback:', e.message);
    // Fallback Claude pour la recherche externe
